...
def mapping_zone_codici(df):
    ...
    if "zona" in df.columns:
        for e in df["zona"].str.split(
                "-"):  # prendo la colonna zona ed effettuo lo split per salvare nella lista nomi_zone
...
def crea_dataframe_rango_unico(filename):
    ...
    df_zona = crea_sub_df("nome_zona", "zone", filename)
    ...
    df = pd.DataFrame()
    df = df_step.merge(df_course, how='left', on="posLin", sort=True)
    ...
    df = df.merge(df_rpm_modificato, how='left', on="posLin", sort=True)
    ...
    df = df.merge(df_zona_modificato, how='left', on="posLin", sort=True)
    ...
    df = df.merge(df_econ_modificato, how='left', on="posLin", sort=True)
    # merge con df_forstep
    df = df.merge(df_art_forstep, how='left', on="posLin", sort=True)
    ...
    df["econ"] = df.apply(lambda row: True if (row["econ"] == False and row["forstep_econ"] != 0) else row["econ"],
                          axis=1)
    ...
    # rimuovi la colonna poslin e forstep_Econ che ora non serve più a nulla. non possibile rimuoverla prima perchè serve per rimozione
    df = df.drop("posLin", axis=1)
    df = df.drop("forstep_econ", axis=1)
    # riordina le colonne
    columns_order = ['step', 'course', 'relative_course', 'econ', 'rpm', 'zone']
